...
const TemporalAnalysis = () => {
  ...
  const handleYearChange = (event) => {
    ...
    if (newYear < 2016 || newYear > 2024) {
      throw new Error(
        "No data available for dates before 2016 and after 2024."
      );
    }
    setYear(newYear);
  };
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/timeData")
      .then((response) => {
  ...
  const incidentsPerMonthSeasonalMapped = Object.keys(
    incidentsPerMonthSeasonal
  ).reduce((counts, month) => {
  ...
  const filteredSeasonalData = data.filter(
    (item) => monthsSeasons[item.month] === selectedSeason
  );
