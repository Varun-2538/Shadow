    ...
    return Object.entries(frequencyData)
      .map(([field, values]) => {
        ...
        const topThreeText = topThreeValues
          ...
          .join(", ");
        const percentageText = topThreeValues
          .map(([_, freq]) => `${Math.floor((freq / total) * 100)}%`)
          .join(", ");
        ...
        return (
          <div key={field} className="mt-4">
            ...
            <p>{topThreeText}</p>
            <p>
              Most of the {field} in this {selectedDistrict} district and{" "}
              {selectedUnit || "entire district"} unit belongs to {topThreeText}{" "}
              where total {field} number is {total} which makes {percentageText}{" "}
              of the total {field}.
            </p>
          </div>
        );
        ...
        <div className="ml-10 mt-5 w-4/5">
          ...
          <div className="mt-8">
            <h2 className="text-xl font-bold mb-4">
              Top 3 Frequencies for Each Field
            </h2>
            {Object.keys(frequencyData).length > 0 &&
              renderTopThreeFrequencies()}
          </div>
        </div>
      </div>
    </>
